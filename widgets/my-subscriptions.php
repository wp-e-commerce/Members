...
class WP_Widget_My_Subscriptions extends WP_Widget {
    ...
    function widget( $args, $instance ) {
        extract( $args );
        $title = apply_filters( 'widget_title', empty( $instance['title'] ) ? __('My Subscriptions', 'wpsc_members') : $instance['title'], $instance, $this->id_base );
...
?>
        <p><label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e('Title:', 'wpsc_members'); ?></label>
...
function wpsc_get_my_subscriptions_kos() {
    ...
    if ( $names != NULL ) {
        ...
        $ouput .= '<td>' . __('Subscription', 'wpsc_members') . '</td>';
        $ouput .= '<td>' . __('Start date', 'wpsc_members') . '</td>';
        $ouput .= '<td>' . __('Expiry date', 'wpsc_members') . '</td>';
        $ouput .= '</tr>';
        ...
        $ouput = '<tr><td colspan="3">' . __('Currently, you have no active subscriptions', 'wpsc_members') . '</td></tr>';
        //return NULL;
        return $ouput;
        return NULL;
    }
...
function wpsc_get_my_subscriptions() {
    ...
    if ( $names != NULL ) {
        ...
        $ouput .= '<li class="subscription-head"><div class="fl">' . __('Title', 'wpsc_members') . '</div><div class="fr">' . __('Expiry Date', 'wpsc_members') . '</div><br clear="all"></li>';
        foreach ( $names as $name ) {
