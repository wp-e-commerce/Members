    ...
    */
    ...
    function get_columns() {
        $columns = array(
            ...
            'subscription'  => __('Current Subscriptions', 'wpsc_members'),
        );
        return $columns;
    }
    ...
    function prepare_items() {
        ...
        $query = "SELECT `ID`, `user_login` FROM " . $wpdb->users . ( ( ! empty( $_GET['s'] ) ) ? " WHERE `user_login` LIKE '%" . $_GET['s'] . "%'" : "" ) . " ORDER BY " . $orderby . " " . $order . " LIMIT " . $start_limit . "," . $end_limit;
        ...
        foreach ( $users as $user ) {
            ...
            if ( ! empty( $subscription_ends ) )
                ...
                $subscriptions[] = __('No Current Subscriptions', 'wpsc_members');
