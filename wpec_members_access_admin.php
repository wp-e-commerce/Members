...
function wpec_members_remove_all_capabilities( $user_id ) {
    ...
    wpec_members_display_update( __('Removed ok.', 'wpsc_members') );
}
...
function wpec_members_save_user( $user_id, $length, $role ) {
    ...
    error_log( "wpec_members_save_user" );
...
function wpec_members_edit_user() {
    ...
    if ( ! isset( $_REQUEST['page'] ) || $_REQUEST['page'] != 'wpec_members' )
        return;
    ...
    if ( $length == 0 ) { // Subscription Deactivated
        ...
        //exit('<pre>'.print_r($_POST,1)."\n".print_r(get_user_meta($user_id, '_subscription_starts'), true).'</pre>');
        $current_subscription_starts = get_user_meta( $user_id, '_subscription_starts' );

        if ( empty($current_subscription_starts ) ) {
            wp_safe_redirect( '?page=wpec_members' );
            return;
        }

        foreach ( $current_subscription_starts[0] as $key => $value ) {
            $current_role = $key;
            $current_start = $value;
        }
        ...
        $new_subscription_ends = array();
        $new_subscription_ends[ $new_role ] = (int) $current_start + (int) $length;

        $new_subscription_starts = array();
        $new_subscription_starts[ $new_role ] = $current_start;

        $new_subscription_length = array();
        $new_subscription_length[ $new_role ] = $length;

        update_user_meta( $user_id, '_subscription_length', $new_subscription_length );
        update_user_meta( $user_id, '_subscription_starts', $new_subscription_starts );
        update_user_meta( $user_id, '_subscription_ends', $new_subscription_ends );
    ...
    switch ( $bulk_option ) {
        case 1:
            if ( empty( $user_ids ) || empty( $length ) || empty( $subscription ) ) {
                wpec_members_display_error( __('You Must select Users, Subscription and a subscription duration to use this bulk option!', 'wpsc_members') );
                return;
            }
            foreach ( $user_ids as $user_id )
        ...
        case 2:
            if ( empty($user_ids) ) {
                wpec_members_display_error( __('You Must select some users in order to remove their subscriptions', 'wpsc_members') );
                return;
            }
            foreach ( $user_ids as $user_id )
                wpec_members_remove_all_capabilities( $user_id );
        ...
        default:
            wpec_members_display_error( __('You must select a bulk option!', 'wpsc_members') );
    }
...
function wpec_members_single_add_wp_users() {
    ...
    if ( empty( $user_id ) || empty( $length ) || empty( $subscription ) ) {
        wpec_members_display_error( __('You must select a subscription and duration to add this user.', 'wpsc_members') );
        return;
    }
...
function wpec_members_delete_subscription( $capability ) {
    ...
    wpec_members_display_update( __('Subscription Deleted', 'wpsc_members') );
}
    ...
    if ( $_POST['page_action'] == 'add' && ! empty( $_POST['capability_list']['new']['capability'] ) && ! empty( $_POST['capability_list']['new']['name'] ) ) {
        ...
        if ( is_array( $wpsc_product_capability_list ) )
            ...
            if ( empty( $wpsc_product_capability_list ) )
                $wpsc_product_capability_list = $new_capability_item;
        else {
...
function wpsc_members_save_settings() {
    ...
    $ipn_error_notification_email = $_POST['ipn_error_notification_email'];
    ...
    if ( is_email( $ipn_error_notification_email ) ) {
        update_site_option( 'ipn_error_notification_email', $ipn_error_notification_email );
    }
}
...
function wpsc_members_admin_css() {
    wp_enqueue_style( 'wpsc_capabilities', WPSC_MEMBERS_URL . '/css-and-js/style.css');
}

add_action( 'admin_init', 'wpsc_members_admin_css');
?>